...
public class WildcardMatch {
  ...
  boolean doMatch(String s, String p, int ls, int lp) {
    ...
    if (doMatch(s, p, ls, lp + 1)) return true;
    while (ls < s.length()) {
      if (doMatch(s, p, ls + 1, lp + 1)) return true;
      ls++;
    }
    return false;
  }
  ...
  boolean doMatch_improved(String s, String p, int ls, int lp) {
    ...
    if (lp == p.length() - 1) return true;
    while (lp < p.length() && p.charAt(lp) == '*') lp++;
    if (lp == p.length()) return ls == s.length();
    char next = p.charAt(lp);
    if (next == '?') {
      if (doMatch(s, p, ls, lp)) return true;
      while (ls < s.length()) {
        if (doMatch(s, p, ls + 1, lp)) return true;
        ls++;
      }
      return false;
    } else {
      if (ls < s.length() && s.charAt(ls) == next && doMatch(s, p, ls, lp)) return true;
      while (ls < s.length()) {
        while (ls < s.length() && s.charAt(ls) != next) ls++;
        if (ls == s.length()) return false;
        if (doMatch(s, p, ls, lp)) return true;
        ls++;
      }
      return false;
