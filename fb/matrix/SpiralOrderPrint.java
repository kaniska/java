...
public class SpiralOrderPrint {
  ...
  void traverse(int[][] matrix, int si, int sj, int ei, int ej, ArrayList<Integer> result) {
    ...
    if (si == ei) {
      for (int j = sj; j <= ej; j++) result.add(matrix[si][j]);
      return;
    }
    if (sj == ej) {
      for (int i = si; i <= ei; i++) result.add(matrix[i][sj]);
      return;
    }
    for (int j = sj; j <= ej; j++) result.add(matrix[si][j]);
    for (int i = si + 1; i < ei; i++) result.add(matrix[i][ej]);
    for (int j = ej; j >= sj; j--) result.add(matrix[ei][j]);
    for (int i = ei - 1; i > si; i--) result.add(matrix[i][sj]);
