...
public class RotateMatrix {
  ...
  public static int[][] rotate90(int[][] m, int algorithm) {
    switch (algorithm) {
      case 0:
        return rotate90a_not_in_place(m);
      case 1:
        return rotate90b_transpose(m);
      case 2:
        return rotate90c_in_place(m);
    }
    return null;
  }
   ...
   * @return
   */
  public static int[][] rotate90a_not_in_place(int[][] m) {
    int rows = m.length;
    int cols = m[0].length;
   ...
   * @return
   */
  public static int[][] rotate90b_transpose(int[][] m) {
    int rows = m.length;
    int cols = m[0].length;
    if (rows != cols) {
      return null;
  ...
  public static int[][] rotate90c_in_place(int[][] m) {
    int rows = m.length;
    int cols = m[0].length;
    if (rows != cols) {
      return null;
  ...
  public static void main(String args[]) {
    ...
    for (Type t : Type.values()) {
      ...
      if (isEqual(r, expected)) {
        System.out.println("CORRECT " + t.name());
      }
      else {
        System.out.println("WRONG " + t.name());
      }
    }
  }
}
