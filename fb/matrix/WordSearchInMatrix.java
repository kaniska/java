...
public class WordSearchInMatrix {
  ...
  public static boolean stringMatch(char[][] m, String s) {
    ...
    for (int i = 0; i < m.length; i++) {
      for (int j = 0; j < m[0].length; j++) {
        if (searchDFS(m, i, j, s, 0, visited)) {
          found = true;
        }
      }
    }
  ...
  public static boolean searchDFS(char[][] m, int x, int y, String s, int pos, boolean[][] v) {
    ...
    if (m[x][y] != s.charAt(pos)) {
      return false;
    }
    v[x][y] = true;
    boolean valid = false;
    if (x > 0 && !v[x - 1][y] && searchDFS(m, x - 1, y, s, pos + 1, v)) {
      valid = true;
    }
    if (valid) {
      return valid;
    }
    if (y > 0 && !v[x][y - 1] && searchDFS(m, x, y - 1, s, pos + 1, v)) {
      valid = true;
    }
    if (valid) {
      return valid;
    }
    if (x < m.length - 1 && !v[x + 1][y] && searchDFS(m, x + 1, y, s, pos + 1, v)) {
      valid = true;
    }
    if (valid) {
      return valid;
    }
    if (y < m[0].length - 1 && !v[x][y + 1] && searchDFS(m, x, y + 1, s, pos + 1, v)) {
      valid = true;
    }
    if (valid) {
      return valid;
