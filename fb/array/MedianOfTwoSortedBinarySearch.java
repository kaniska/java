...
//    if (len == 1) return (x + A[0]) / 2.0d;
  ...
  private double findMedianSortedArray(int A[], int B[], int l1, int r1, int l2, int r2) {
    ...
    if (len1 == 1) {
      return findMedianWithOne(A[0], B);
    }
    if (len2 == 1) {
      return findMedianWithOne(B[0], A);
    }
    if (len1 == 2) {
      return findMedianWithTwo(A[0], A[1], B);
    }
    if (len2 == 2) {
      return findMedianWithTwo(B[0], B[1], A);
    }
